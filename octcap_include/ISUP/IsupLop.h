...
//  Revision: 
//            $Source: /ISUP/code/message/IsupLop.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupLop : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP LOP (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[LOP message]>.
{
public:

  // = CONSTRUCTION

  IsupLop(ISUP::MsgSetID);
  // This constructor builds an empty LOP message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the LOP as defined in the load metadata will
  ...
  // constructor, a call to <{IsupLop::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the LOP message.

  virtual ~IsupLop();
  // This destructor deletes a LOP message. 

  static IsupLop* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupLop}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupLop(const IsupLop& P_from);
  // The copy constructor provides a way to construct a LOP message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupLop}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing LOP parameters. Each 
  ...
  IsupLop* messageCompatibility(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupLop* parameterCompatibility(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupLop* callTransferReference(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupLop* loopPreventionIndicator(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupLop& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupLop}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupLop* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupLop}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupLop& operator=(const IsupLop&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupLop static data 

  static void end();
  // Ask to end with the IsupLop static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message LOP which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the LOP message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new LOP messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the LOP messages.
...
// = EXAMPLE
//
// This example shows how a message LOP can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupLop is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupLop::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupLop* msgPtr = NEW IsupLop(msgSetId);
