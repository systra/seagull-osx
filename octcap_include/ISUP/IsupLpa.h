...
//  Revision: 
//            $Source: /ISUP/code/message/IsupLpa.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupLpa : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP LPA (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[LPA message]>.
{
public:

  // = CONSTRUCTION

  IsupLpa(ISUP::MsgSetID);
  // This constructor builds an empty LPA message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the LPA as defined in the load metadata will
  ...
  // constructor, a call to <{IsupLpa::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the LPA message.

  virtual ~IsupLpa();
  // This destructor deletes a LPA message. 

  static IsupLpa* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupLpa}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupLpa(const IsupLpa& P_from);
  // The copy constructor provides a way to construct a LPA message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupLpa}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing LPA parameters. Each 
  ...
  friend ostream& operator<< (ostream& P_os, const IsupLpa& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupLpa}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupLpa* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupLpa}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupLpa& operator=(const IsupLpa&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupLpa static data 

  static void end();
  // Ask to end with the IsupLpa static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message LPA which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the LPA message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new LPA messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the LPA messages.
...
// = EXAMPLE
//
// This example shows how a message LPA can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupLpa is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupLpa::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupLpa* msgPtr = NEW IsupLpa(msgSetId);
