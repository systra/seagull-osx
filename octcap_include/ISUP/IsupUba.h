...
//  Revision: 
//            $Source: /ISUP/code/message/IsupUba.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupUba : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP UBA (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[UBA message]>.
{
public:

  // = CONSTRUCTION

  IsupUba(ISUP::MsgSetID);
  // This constructor builds an empty UBA message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the UBA as defined in the load metadata will
  ...
  // constructor, a call to <{IsupUba::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the UBA message.

  virtual ~IsupUba();
  // This destructor deletes a UBA message. 

  static IsupUba* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupUba}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupUba(const IsupUba& P_from);
  // The copy constructor provides a way to construct a UBA message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupUba}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing UBA parameters. Each 
  ...
  friend ostream& operator<< (ostream& P_os, const IsupUba& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupUba}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupUba* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupUba}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupUba& operator=(const IsupUba&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupUba static data 

  static void end();
  // Ask to end with the IsupUba static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message UBA which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the UBA message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new UBA messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the UBA messages.
...
// = EXAMPLE
//
// This example shows how a message UBA can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupUba is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupUba::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupUba* msgPtr = NEW IsupUba(msgSetId);
