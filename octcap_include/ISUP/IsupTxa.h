...
//  Revision: 
//            $Source: /ISUP/code/message/IsupTxa.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupTxa : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP TXA (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[TXA message]>.
{
public:

  // = CONSTRUCTION

  IsupTxa(ISUP::MsgSetID);
  // This constructor builds an empty TXA message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the TXA as defined in the load metadata will
  ...
  // constructor, a call to <{IsupTxa::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the TXA message.

  virtual ~IsupTxa();
  // This destructor deletes a TXA message. 

  static IsupTxa* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupTxa}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupTxa(const IsupTxa& P_from);
  // The copy constructor provides a way to construct a TXA message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupTxa}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing TXA parameters. Each 
  ...
  IsupTxa* parameterCompatibility(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupTxa& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupTxa}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupTxa* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupTxa}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupTxa& operator=(const IsupTxa&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupTxa static data 

  static void end();
  // Ask to end with the IsupTxa static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message TXA which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the TXA message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new TXA messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the TXA messages.
...
// = EXAMPLE
//
// This example shows how a message TXA can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupTxa is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupTxa::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupTxa* msgPtr = NEW IsupTxa(msgSetId);
