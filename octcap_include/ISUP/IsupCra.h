...
//  Revision: 
//            $Source: /ISUP/code/message/IsupCra.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupCra : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP CRA (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[CRA message]>.
{
public:

  // = CONSTRUCTION

  IsupCra(ISUP::MsgSetID);
  // This constructor builds an empty CRA message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the CRA as defined in the load metadata will
  ...
  // constructor, a call to <{IsupCra::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the CRA message.

  virtual ~IsupCra();
  // This destructor deletes a CRA message. 

  static IsupCra* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupCra}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupCra(const IsupCra& P_from);
  // The copy constructor provides a way to construct a CRA message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupCra}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing CRA parameters. Each 
  ...
  friend ostream& operator<< (ostream& P_os, const IsupCra& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupCra}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupCra* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupCra}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupCra& operator=(const IsupCra&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupCra static data 

  static void end();
  // Ask to end with the IsupCra static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message CRA which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the CRA message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new CRA messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the CRA messages.
...
// = EXAMPLE
//
// This example shows how a message CRA can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupCra is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupCra::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupCra* msgPtr = NEW IsupCra(msgSetId);
