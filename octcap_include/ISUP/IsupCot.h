...
//  Revision: 
//            $Source: /ISUP/code/message/IsupCot.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupCot : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP COT (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[COT message]>.
{
public:

  // = CONSTRUCTION

  IsupCot(ISUP::MsgSetID);
  // This constructor builds an empty COT message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the COT as defined in the load metadata will
  ...
  // constructor, a call to <{IsupCot::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the COT message.

  virtual ~IsupCot();
  // This destructor deletes a COT message. 

  static IsupCot* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupCot}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupCot(const IsupCot& P_from);
  // The copy constructor provides a way to construct a COT message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupCot}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing COT parameters. Each 
  ...
  IsupCot* continuityIndicators(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupCot& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupCot}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupCot* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupCot}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupCot& operator=(const IsupCot&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupCot static data 

  static void end();
  // Ask to end with the IsupCot static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message COT which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the COT message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new COT messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the COT messages.
...
// = EXAMPLE
//
// This example shows how a message COT can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupCot is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupCot::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupCot* msgPtr = NEW IsupCot(msgSetId);
