...
//  Revision: 
//            $Source: /ISUP/code/message/IsupCmr.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupCmr : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP CMR (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[CMR message]>.
{
public:

  // = CONSTRUCTION

  IsupCmr(ISUP::MsgSetID);
  // This constructor builds an empty CMR message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the CMR as defined in the load metadata will
  ...
  // constructor, a call to <{IsupCmr::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the CMR message.

  virtual ~IsupCmr();
  // This destructor deletes a CMR message. 

  static IsupCmr* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupCmr}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupCmr(const IsupCmr& P_from);
  // The copy constructor provides a way to construct a CMR message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupCmr}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing CMR parameters. Each 
  ...
  IsupCmr* callModificationIndicators(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupCmr* callReference(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupCmr* userToUserInformation(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupCmr& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupCmr}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupCmr* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupCmr}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupCmr& operator=(const IsupCmr&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupCmr static data 

  static void end();
  // Ask to end with the IsupCmr static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message CMR which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the CMR message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new CMR messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the CMR messages.
...
// = EXAMPLE
//
// This example shows how a message CMR can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupCmr is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupCmr::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupCmr* msgPtr = NEW IsupCmr(msgSetId);
