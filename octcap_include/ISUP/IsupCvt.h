...
//  Revision: 
//            $Source: /ISUP/code/message/IsupCvt.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupCvt : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP CVT (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[CVT message]>.
{
public:

  // = CONSTRUCTION

  IsupCvt(ISUP::MsgSetID);
  // This constructor builds an empty CVT message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the CVT as defined in the load metadata will
  ...
  // constructor, a call to <{IsupCvt::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the CVT message.

  virtual ~IsupCvt();
  // This destructor deletes a CVT message. 

  static IsupCvt* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupCvt}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupCvt(const IsupCvt& P_from);
  // The copy constructor provides a way to construct a CVT message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupCvt}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing CVT parameters. Each 
  ...
  friend ostream& operator<< (ostream& P_os, const IsupCvt& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupCvt}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupCvt* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupCvt}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupCvt& operator=(const IsupCvt&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupCvt static data 

  static void end();
  // Ask to end with the IsupCvt static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message CVT which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the CVT message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new CVT messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the CVT messages.
...
// = EXAMPLE
//
// This example shows how a message CVT can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupCvt is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupCvt::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupCvt* msgPtr = NEW IsupCvt(msgSetId);
