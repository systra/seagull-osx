...
//  Revision: 
//            $Source: /ISUP/code/message/IsupSus.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupSus : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP SUS (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[SUS message]>.
{
public:

  // = CONSTRUCTION

  IsupSus(ISUP::MsgSetID);
  // This constructor builds an empty SUS message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the SUS as defined in the load metadata will
  ...
  // constructor, a call to <{IsupSus::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the SUS message.

  virtual ~IsupSus();
  // This destructor deletes a SUS message. 

  static IsupSus* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupSus}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupSus(const IsupSus& P_from);
  // The copy constructor provides a way to construct a SUS message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupSus}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing SUS parameters. Each 
  ...
  IsupSus* suspendResumeIndicators(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupSus* callReference(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupSus* mobileCommEndInfoTrf(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupSus& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupSus}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupSus* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupSus}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupSus& operator=(const IsupSus&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupSus static data 

  static void end();
  // Ask to end with the IsupSus static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message SUS which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the SUS message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new SUS messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the SUS messages.
...
// = EXAMPLE
//
// This example shows how a message SUS can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupSus is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupSus::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupSus* msgPtr = NEW IsupSus(msgSetId);
