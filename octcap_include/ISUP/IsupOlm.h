...
//  Revision: 
//            $Source: /ISUP/code/message/IsupOlm.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupOlm : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP OLM (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[OLM message]>.
{
public:

  // = CONSTRUCTION

  IsupOlm(ISUP::MsgSetID);
  // This constructor builds an empty OLM message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the OLM as defined in the load metadata will
  ...
  // constructor, a call to <{IsupOlm::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the OLM message.

  virtual ~IsupOlm();
  // This destructor deletes a OLM message. 

  static IsupOlm* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupOlm}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupOlm(const IsupOlm& P_from);
  // The copy constructor provides a way to construct a OLM message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupOlm}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing OLM parameters. Each 
  ...
  friend ostream& operator<< (ostream& P_os, const IsupOlm& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupOlm}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupOlm* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupOlm}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupOlm& operator=(const IsupOlm&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupOlm static data 

  static void end();
  // Ask to end with the IsupOlm static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message OLM which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the OLM message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new OLM messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the OLM messages.
...
// = EXAMPLE
//
// This example shows how a message OLM can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupOlm is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupOlm::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupOlm* msgPtr = NEW IsupOlm(msgSetId);
