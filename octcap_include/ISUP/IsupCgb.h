...
//  Revision: 
//            $Source: /ISUP/code/message/IsupCgb.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupCgb : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP CGB (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[CGB message]>.
{
public:

  // = CONSTRUCTION

  IsupCgb(ISUP::MsgSetID);
  // This constructor builds an empty CGB message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the CGB as defined in the load metadata will
  ...
  // constructor, a call to <{IsupCgb::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the CGB message.

  virtual ~IsupCgb();
  // This destructor deletes a CGB message. 

  static IsupCgb* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupCgb}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupCgb(const IsupCgb& P_from);
  // The copy constructor provides a way to construct a CGB message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupCgb}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing CGB parameters. Each 
  ...
  IsupCgb* cirGroupSupervMessageTypeInd(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupCgb* rangeAndStatus(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupCgb& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupCgb}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupCgb* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupCgb}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupCgb& operator=(const IsupCgb&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupCgb static data 

  static void end();
  // Ask to end with the IsupCgb static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message CGB which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the CGB message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new CGB messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the CGB messages.
...
// = EXAMPLE
//
// This example shows how a message CGB can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupCgb is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupCgb::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupCgb* msgPtr = NEW IsupCgb(msgSetId);
