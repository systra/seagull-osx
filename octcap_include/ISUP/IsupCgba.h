...
//  Revision: 
//            $Source: /ISUP/code/message/IsupCgba.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupCgba : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP CGBA (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[CGBA message]>.
{
public:

  // = CONSTRUCTION

  IsupCgba(ISUP::MsgSetID);
  // This constructor builds an empty CGBA message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the CGBA as defined in the load metadata will
  ...
  // constructor, a call to <{IsupCgba::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the CGBA message.

  virtual ~IsupCgba();
  // This destructor deletes a CGBA message. 

  static IsupCgba* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupCgba}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupCgba(const IsupCgba& P_from);
  // The copy constructor provides a way to construct a CGBA message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupCgba}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing CGBA parameters. Each 
  ...
  IsupCgba* cirGroupSupervMessageTypeInd(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupCgba* rangeAndStatus(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupCgba& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupCgba}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupCgba* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupCgba}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupCgba& operator=(const IsupCgba&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupCgba static data 

  static void end();
  // Ask to end with the IsupCgba static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message CGBA which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the CGBA message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new CGBA messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the CGBA messages.
...
// = EXAMPLE
//
// This example shows how a message CGBA can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupCgba is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupCgba::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupCgba* msgPtr = NEW IsupCgba(msgSetId);
