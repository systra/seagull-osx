...
//  Revision: 
//            $Source: /ISUP/code/message/IsupUpt.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupUpt : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP UPT (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[UPT message]>.
{
public:

  // = CONSTRUCTION

  IsupUpt(ISUP::MsgSetID);
  // This constructor builds an empty UPT message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the UPT as defined in the load metadata will
  ...
  // constructor, a call to <{IsupUpt::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the UPT message.

  virtual ~IsupUpt();
  // This destructor deletes a UPT message. 

  static IsupUpt* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupUpt}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupUpt(const IsupUpt& P_from);
  // The copy constructor provides a way to construct a UPT message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupUpt}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing UPT parameters. Each 
  ...
  IsupUpt* parameterCompatibility(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupUpt* messageCompatibility(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupUpt& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupUpt}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupUpt* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupUpt}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupUpt& operator=(const IsupUpt&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupUpt static data 

  static void end();
  // Ask to end with the IsupUpt static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message UPT which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the UPT message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new UPT messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the UPT messages.
...
// = EXAMPLE
//
// This example shows how a message UPT can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupUpt is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupUpt::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupUpt* msgPtr = NEW IsupUpt(msgSetId);
