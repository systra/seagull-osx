...
//  Revision: 
//            $Source: /ISUP/code/message/IsupBlo.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupBlo : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP BLO (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[BLO message]>.
{
public:

  // = CONSTRUCTION

  IsupBlo(ISUP::MsgSetID);
  // This constructor builds an empty BLO message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the BLO as defined in the load metadata will
  ...
  // constructor, a call to <{IsupBlo::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the BLO message.

  virtual ~IsupBlo();
  // This destructor deletes a BLO message. 

  static IsupBlo* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupBlo}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupBlo(const IsupBlo& P_from);
  // The copy constructor provides a way to construct a BLO message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupBlo}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing BLO parameters. Each 
  ...
  friend ostream& operator<< (ostream& P_os, const IsupBlo& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupBlo}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupBlo* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupBlo}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupBlo& operator=(const IsupBlo&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupBlo static data 

  static void end();
  // Ask to end with the IsupBlo static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message BLO which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the BLO message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new BLO messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the BLO messages.
...
// = EXAMPLE
//
// This example shows how a message BLO can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupBlo is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupBlo::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupBlo* msgPtr = NEW IsupBlo(msgSetId);
