...
//  Revision: 
//            $Source: /ISUP/code/message/IsupCgua.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupCgua : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP CGUA (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[CGUA message]>.
{
public:

  // = CONSTRUCTION

  IsupCgua(ISUP::MsgSetID);
  // This constructor builds an empty CGUA message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the CGUA as defined in the load metadata will
  ...
  // constructor, a call to <{IsupCgua::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the CGUA message.

  virtual ~IsupCgua();
  // This destructor deletes a CGUA message. 

  static IsupCgua* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupCgua}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupCgua(const IsupCgua& P_from);
  // The copy constructor provides a way to construct a CGUA message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupCgua}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing CGUA parameters. Each 
  ...
  IsupCgua* cirGroupSupervMessageTypeInd(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupCgua* rangeAndStatus(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupCgua& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupCgua}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupCgua* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupCgua}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupCgua& operator=(const IsupCgua&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupCgua static data 

  static void end();
  // Ask to end with the IsupCgua static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message CGUA which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the CGUA message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new CGUA messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the CGUA messages.
...
// = EXAMPLE
//
// This example shows how a message CGUA can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupCgua is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupCgua::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupCgua* msgPtr = NEW IsupCgua(msgSetId);
