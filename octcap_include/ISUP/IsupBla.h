...
//  Revision: 
//            $Source: /ISUP/code/message/IsupBla.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupBla : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP BLA (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[BLA message]>.
{
public:

  // = CONSTRUCTION

  IsupBla(ISUP::MsgSetID);
  // This constructor builds an empty BLA message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the BLA as defined in the load metadata will
  ...
  // constructor, a call to <{IsupBla::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the BLA message.

  virtual ~IsupBla();
  // This destructor deletes a BLA message. 

  static IsupBla* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupBla}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupBla(const IsupBla& P_from);
  // The copy constructor provides a way to construct a BLA message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupBla}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing BLA parameters. Each 
  ...
  friend ostream& operator<< (ostream& P_os, const IsupBla& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupBla}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupBla* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupBla}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupBla& operator=(const IsupBla&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupBla static data 

  static void end();
  // Ask to end with the IsupBla static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message BLA which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the BLA message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new BLA messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the BLA messages.
...
// = EXAMPLE
//
// This example shows how a message BLA can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupBla is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupBla::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupBla* msgPtr = NEW IsupBla(msgSetId);
