...
//  Revision: 
//            $Source: /ISUP/code/message/IsupDrs.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupDrs : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP DRS (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[DRS message]>.
{
public:

  // = CONSTRUCTION

  IsupDrs(ISUP::MsgSetID);
  // This constructor builds an empty DRS message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the DRS as defined in the load metadata will
  ...
  // constructor, a call to <{IsupDrs::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the DRS message.

  virtual ~IsupDrs();
  // This destructor deletes a DRS message. 

  static IsupDrs* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupDrs}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupDrs(const IsupDrs& P_from);
  // The copy constructor provides a way to construct a DRS message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupDrs}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing DRS parameters. Each 
  ...
  IsupDrs* callReference(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupDrs& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupDrs}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupDrs* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupDrs}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupDrs& operator=(const IsupDrs&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupDrs static data 

  static void end();
  // Ask to end with the IsupDrs static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message DRS which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the DRS message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new DRS messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the DRS messages.
...
// = EXAMPLE
//
// This example shows how a message DRS can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupDrs is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupDrs::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupDrs* msgPtr = NEW IsupDrs(msgSetId);
