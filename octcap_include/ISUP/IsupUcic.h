...
//  Revision: 
//            $Source: /ISUP/code/message/IsupUcic.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupUcic : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP UCIC (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[UCIC message]>.
{
public:

  // = CONSTRUCTION

  IsupUcic(ISUP::MsgSetID);
  // This constructor builds an empty UCIC message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the UCIC as defined in the load metadata will
  ...
  // constructor, a call to <{IsupUcic::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the UCIC message.

  virtual ~IsupUcic();
  // This destructor deletes a UCIC message. 

  static IsupUcic* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupUcic}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupUcic(const IsupUcic& P_from);
  // The copy constructor provides a way to construct a UCIC message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupUcic}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing UCIC parameters. Each 
  ...
  friend ostream& operator<< (ostream& P_os, const IsupUcic& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupUcic}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupUcic* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupUcic}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupUcic& operator=(const IsupUcic&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupUcic static data 

  static void end();
  // Ask to end with the IsupUcic static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message UCIC which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the UCIC message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new UCIC messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the UCIC messages.
...
// = EXAMPLE
//
// This example shows how a message UCIC can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupUcic is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupUcic::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupUcic* msgPtr = NEW IsupUcic(msgSetId);
