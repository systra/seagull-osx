...
//  Revision: 
//            $Source: /ISUP/code/message/IsupRsc.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupRsc : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP RSC (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[RSC message]>.
{
public:

  // = CONSTRUCTION

  IsupRsc(ISUP::MsgSetID);
  // This constructor builds an empty RSC message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the RSC as defined in the load metadata will
  ...
  // constructor, a call to <{IsupRsc::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the RSC message.

  virtual ~IsupRsc();
  // This destructor deletes a RSC message. 

  static IsupRsc* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupRsc}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupRsc(const IsupRsc& P_from);
  // The copy constructor provides a way to construct a RSC message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupRsc}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing RSC parameters. Each 
  ...
  friend ostream& operator<< (ostream& P_os, const IsupRsc& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupRsc}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupRsc* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupRsc}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupRsc& operator=(const IsupRsc&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupRsc static data 

  static void end();
  // Ask to end with the IsupRsc static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message RSC which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the RSC message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new RSC messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the RSC messages.
...
// = EXAMPLE
//
// This example shows how a message RSC can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupRsc is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupRsc::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupRsc* msgPtr = NEW IsupRsc(msgSetId);
