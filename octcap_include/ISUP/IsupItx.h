...
//  Revision: 
//            $Source: /ISUP/code/message/IsupItx.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupItx : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP ITX (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[ITX message]>.
{
public:

  // = CONSTRUCTION

  IsupItx(ISUP::MsgSetID);
  // This constructor builds an empty ITX message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the ITX as defined in the load metadata will
  ...
  // constructor, a call to <{IsupItx::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the ITX message.

  virtual ~IsupItx();
  // This destructor deletes a ITX message. 

  static IsupItx* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupItx}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupItx(const IsupItx& P_from);
  // The copy constructor provides a way to construct a ITX message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupItx}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing ITX parameters. Each 
  ...
  IsupItx* chargeUnitNumber(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupItx* messageNumber(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  ISUP::ParmValue* taxChargingField(ISUP::MsgStatus& P_status) const;
  // Provides read access to the <{taxChargingField}>
  // parameter.
  // The user must delete the <{ISUP::ParmValue}> object instance.

  IsupItx* taxChargingField(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  // Provides write access to the <{taxChargingField}>
  // parameter. Setting a value activates the presence flag.
  // This methods does not delete the provided <{ISUP::ParmValue}> instance.
  ...
  IsupItx* parameterCompatibility(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupItx& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupItx}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupItx* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupItx}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupItx& operator=(const IsupItx&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupItx static data 

  static void end();
  // Ask to end with the IsupItx static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message ITX which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the ITX message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new ITX messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the ITX messages.
...
// = EXAMPLE
//
// This example shows how a message ITX can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupItx is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupItx::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupItx* msgPtr = NEW IsupItx(msgSetId);
