...
//  Revision: 
//            $Source: /ISUP/code/message/IsupFaa.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupFaa : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP FAA (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[FAA message]>.
{
public:

  // = CONSTRUCTION

  IsupFaa(ISUP::MsgSetID);
  // This constructor builds an empty FAA message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the FAA as defined in the load metadata will
  ...
  // constructor, a call to <{IsupFaa::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the FAA message.

  virtual ~IsupFaa();
  // This destructor deletes a FAA message. 

  static IsupFaa* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupFaa}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupFaa(const IsupFaa& P_from);
  // The copy constructor provides a way to construct a FAA message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupFaa}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing FAA parameters. Each 
  ...
  IsupFaa* facilityIndicator(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupFaa* userToUserIndicators(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupFaa* callReference(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupFaa* connectionRequest(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupFaa* parameterCompatibility(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupFaa& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupFaa}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupFaa* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupFaa}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupFaa& operator=(const IsupFaa&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupFaa static data 

  static void end();
  // Ask to end with the IsupFaa static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message FAA which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the FAA message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new FAA messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the FAA messages.
...
// = EXAMPLE
//
// This example shows how a message FAA can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupFaa is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupFaa::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupFaa* msgPtr = NEW IsupFaa(msgSetId);
