...
//  Revision: 
//            $Source: /ISUP/code/message/IsupInr.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupInr : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP INR (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[INR message]>.
{
public:

  // = CONSTRUCTION

  IsupInr(ISUP::MsgSetID);
  // This constructor builds an empty INR message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the INR as defined in the load metadata will
  ...
  // constructor, a call to <{IsupInr::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the INR message.

  virtual ~IsupInr();
  // This destructor deletes a INR message. 

  static IsupInr* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupInr}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupInr(const IsupInr& P_from);
  // The copy constructor provides a way to construct a INR message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupInr}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing INR parameters. Each 
  ...
  IsupInr* informationRequestIndicators(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupInr* callReference(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupInr* connectionRequest(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupInr* networkTransport(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupInr* parameterCompatibility(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupInr* networkSpecificFacility(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupInr* unRecognizedParameter(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  HPAIN::Boolean nationalInformationRequestIndicatorIsPresent(ISUP::MsgStatus& P_status) const;
  // Returns TRUE is the optional parameter <{nationalInformationRequestIndicator}>
  // is present in message. Note that no write access is provided : only
  // the setting of a value activates the presence flag.

  void  nationalInformationRequestIndicatorSetAbsent(ISUP::MsgStatus& P_status);
  // Sets the optional parameter <{nationalInformationRequestIndicator}> as absent
  // in the message. 

  ISUP::ParmValue* nationalInformationRequestIndicator(ISUP::MsgStatus& P_status) const;
  // Provides read access to the <{nationalInformationRequestIndicator}>
  // parameter.
  // The user must delete the <{ISUP::ParmValue}> object instance.

  IsupInr* nationalInformationRequestIndicator(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  // Provides write access to the <{nationalInformationRequestIndicator}>
  // parameter. Setting a value activates the presence flag.
  // This methods does not delete the provided <{ISUP::ParmValue}> instance.
  ...
  friend ostream& operator<< (ostream& P_os, const IsupInr& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupInr}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupInr* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupInr}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupInr& operator=(const IsupInr&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupInr static data 

  static void end();
  // Ask to end with the IsupInr static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message INR which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the INR message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new INR messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the INR messages.
...
// = EXAMPLE
//
// This example shows how a message INR can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupInr is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupInr::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupInr* msgPtr = NEW IsupInr(msgSetId);
