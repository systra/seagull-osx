...
//  Revision: 
//            $Source: /ISUP/code/message/IsupUbl.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupUbl : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP UBL (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[UBL message]>.
{
public:

  // = CONSTRUCTION

  IsupUbl(ISUP::MsgSetID);
  // This constructor builds an empty UBL message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the UBL as defined in the load metadata will
  ...
  // constructor, a call to <{IsupUbl::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the UBL message.

  virtual ~IsupUbl();
  // This destructor deletes a UBL message. 

  static IsupUbl* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupUbl}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupUbl(const IsupUbl& P_from);
  // The copy constructor provides a way to construct a UBL message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupUbl}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing UBL parameters. Each 
  ...
  friend ostream& operator<< (ostream& P_os, const IsupUbl& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupUbl}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupUbl* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupUbl}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupUbl& operator=(const IsupUbl&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupUbl static data 

  static void end();
  // Ask to end with the IsupUbl static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message UBL which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the UBL message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new UBL messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the UBL messages.
...
// = EXAMPLE
//
// This example shows how a message UBL can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupUbl is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupUbl::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupUbl* msgPtr = NEW IsupUbl(msgSetId);
