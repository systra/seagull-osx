...
//  Revision: 
//            $Source: /ISUP/code/message/IsupCqr.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupCqr : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP CQR (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[CQR message]>.
{
public:

  // = CONSTRUCTION

  IsupCqr(ISUP::MsgSetID);
  // This constructor builds an empty CQR message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the CQR as defined in the load metadata will
  ...
  // constructor, a call to <{IsupCqr::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the CQR message.

  virtual ~IsupCqr();
  // This destructor deletes a CQR message. 

  static IsupCqr* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupCqr}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupCqr(const IsupCqr& P_from);
  // The copy constructor provides a way to construct a CQR message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupCqr}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing CQR parameters. Each 
  ...
  IsupCqr* rangeAndStatus(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupCqr* circuitStateIndicator(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupCqr& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupCqr}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupCqr* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupCqr}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupCqr& operator=(const IsupCqr&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupCqr static data 

  static void end();
  // Ask to end with the IsupCqr static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message CQR which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the CQR message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new CQR messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the CQR messages.
...
// = EXAMPLE
//
// This example shows how a message CQR can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupCqr is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupCqr::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupCqr* msgPtr = NEW IsupCqr(msgSetId);
