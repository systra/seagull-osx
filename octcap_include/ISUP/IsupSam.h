...
//  Revision: 
//            $Source: /ISUP/code/message/IsupSam.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupSam : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP SAM (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[SAM message]>.
{
public:

  // = CONSTRUCTION

  IsupSam(ISUP::MsgSetID);
  // This constructor builds an empty SAM message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the SAM as defined in the load metadata will
  ...
  // constructor, a call to <{IsupSam::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the SAM message.

  virtual ~IsupSam();
  // This destructor deletes a SAM message. 

  static IsupSam* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupSam}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupSam(const IsupSam& P_from);
  // The copy constructor provides a way to construct a SAM message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupSam}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing SAM parameters. Each 
  ...
  IsupSam* subsequentNumber(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupSam& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupSam}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupSam* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupSam}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupSam& operator=(const IsupSam&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupSam static data 

  static void end();
  // Ask to end with the IsupSam static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message SAM which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the SAM message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new SAM messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the SAM messages.
...
// = EXAMPLE
//
// This example shows how a message SAM can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupSam is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupSam::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupSam* msgPtr = NEW IsupSam(msgSetId);
