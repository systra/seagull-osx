...
//  Revision: 
//            $Source: /ISUP/code/message/IsupExm.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupExm : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP EXM (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[EXM message]>.
{
public:

  // = CONSTRUCTION

  IsupExm(ISUP::MsgSetID);
  // This constructor builds an empty EXM message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the EXM as defined in the load metadata will
  ...
  // constructor, a call to <{IsupExm::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the EXM message.

  virtual ~IsupExm();
  // This destructor deletes a EXM message. 

  static IsupExm* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupExm}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupExm(const IsupExm& P_from);
  // The copy constructor provides a way to construct a EXM message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupExm}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing EXM parameters. Each 
  ...
  HPAIN::Boolean outgoingTrunkGroupNumberIsPresent(ISUP::MsgStatus& P_status) const;
  // Returns TRUE is the optional parameter <{outgoingTrunkGroupNumber}>
  // is present in message. Note that no write access is provided : only
  // the setting of a value activates the presence flag.

  void  outgoingTrunkGroupNumberSetAbsent(ISUP::MsgStatus& P_status);
  // Sets the optional parameter <{outgoingTrunkGroupNumber}> as absent
  // in the message. 

  ISUP::ParmValue* outgoingTrunkGroupNumber(ISUP::MsgStatus& P_status) const;
  // Provides read access to the <{outgoingTrunkGroupNumber}>
  // parameter.
  // The user must delete the <{ISUP::ParmValue}> object instance.

  IsupExm* outgoingTrunkGroupNumber(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  // Provides write access to the <{outgoingTrunkGroupNumber}>
  ...
  IsupExm* unRecognizedParameter(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupExm& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupExm}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupExm* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupExm}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupExm& operator=(const IsupExm&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupExm static data 

  static void end();
  // Ask to end with the IsupExm static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message EXM which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the EXM message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new EXM messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the EXM messages.
...
// = EXAMPLE
//
// This example shows how a message EXM can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupExm is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupExm::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupExm* msgPtr = NEW IsupExm(msgSetId);
