...
//  Revision: 
//            $Source: /ISUP/code/message/IsupUsr.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupUsr : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP USR (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[USR message]>.
{
public:

  // = CONSTRUCTION

  IsupUsr(ISUP::MsgSetID);
  // This constructor builds an empty USR message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the USR as defined in the load metadata will
  ...
  // constructor, a call to <{IsupUsr::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the USR message.

  virtual ~IsupUsr();
  // This destructor deletes a USR message. 

  static IsupUsr* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupUsr}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupUsr(const IsupUsr& P_from);
  // The copy constructor provides a way to construct a USR message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupUsr}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing USR parameters. Each 
  ...
  IsupUsr* userToUserInformation(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupUsr* accessTransport(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupUsr* callReference(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupUsr& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupUsr}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupUsr* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupUsr}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupUsr& operator=(const IsupUsr&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupUsr static data 

  static void end();
  // Ask to end with the IsupUsr static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message USR which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the USR message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new USR messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the USR messages.
...
// = EXAMPLE
//
// This example shows how a message USR can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupUsr is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupUsr::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupUsr* msgPtr = NEW IsupUsr(msgSetId);
