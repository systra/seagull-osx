...
//  Revision: 
//            $Source: /ISUP/code/message/IsupCmrj.h $
//            $Revision: 1.5 $
//            
//
//------------------------------------------------------------------------------
...
class IsupCmrj : public IsupMsg
//
// = TITLE
// C++ encapsulation of an ISUP CMRJ (<{VERSION 2.0}>).
// 
// = DESCRIPTION
// This message class provides a specific interface to individually build and 
// access each parameter of an <[CMRJ message]>.
{
public:

  // = CONSTRUCTION

  IsupCmrj(ISUP::MsgSetID);
  // This constructor builds an empty CMRJ message. The parameter <{P_msgSetId}>,
  // previously allocated by a call to <{IsupMgr::whichMsgSet()}>, must be 
  // passed. Therefore, the CMRJ as defined in the load metadata will
  ...
  // constructor, a call to <{IsupCmrj::isValid()}> inspects the state of the
  // object.
  // From an implementation point of view, this constructor gets an instance
  // of a local representation of the CMRJ message.

  virtual ~IsupCmrj();
  // This destructor deletes a CMRJ message. 

  static IsupCmrj* castMsg(const IsupMsg* P_msg) ;
  ...
  // to get access to all the <{IsupCmrj}> class accessors. If the message set
  // customization options are used, the same message set as the one used by
  // <{P_msgPtr}> will be used.

  IsupCmrj(const IsupCmrj& P_from);
  // The copy constructor provides a way to construct a CMRJ message and does
  ...
  // This method makes the new parameter <{P_parmName}> accessible via the <{IsupCmrj}>
  ...
  // = SPECIFIC ACCESSORS
  // Accessors provide a generic facility for accessing CMRJ parameters. Each 
  ...
  IsupCmrj* callModificationIndicators(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupCmrj* callReference(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  IsupCmrj* userToUserInformation(ISUP::ParmValue& P_val, ISUP::MsgStatus& P_status);
  ...
  friend ostream& operator<< (ostream& P_os, const IsupCmrj& P_msg);
  // Do a pretty output on <{ostream}> of a reference to a <{IsupCmrj}> instance.

  friend ostream& operator<< (ostream& P_os, const IsupCmrj* P_msg);
  // Do a pretty output on <{ostream}> of a pointer to a <{IsupCmrj}> instance.

private:

  // = CONSTRUCTION - INITIALIZATION

  const IsupCmrj& operator=(const IsupCmrj&);
  // The assignment is protected to prevent users from calling it

  static ISUP::InfoStatus init(HPAIN::Uint16 P_NumberOfMsgSet);
  // Ask to initialise the IsupCmrj static data 

  static void end();
  // Ask to end with the IsupCmrj static data 

  // = STANDARD MESSAGE MANAGEMENT
  
  static ISUP::InfoStatus installStdMsg(ISUP::MsgSetID P_msgSetId,
					HPAIN::Int32& P_tag);
  // Ask to support the standard message CMRJ which is belonging to
  // message set <{P_msgSetId}>.

  static const BF_MDMsg* getMsg(ISUP::MsgSetID P_msgSetId);
  // Ask the related BF reference of the CMRJ message.

  static HPAIN::Uint16 numberOfMsgSet;
  // number of message sets supported by this class.

  static const BF_MDMsg** relatedMsg;
  // Table containing the new CMRJ messages.

  static const BF_MDMsgElem*** relatedMsgElem;
  // Table containing the new parameters of the CMRJ messages.
...
// = EXAMPLE
//
// This example shows how a message CMRJ can be accessed.
//
// = BEGIN<CODE>
//
...
//
//  // IsupCmrj is a standard derived class of IsupMsg
//  // This message has a new parameter : NEW_PARM
//  install = IsupCmrj::installParm ("NEW_PARM", msgSetId, parmId);
//  if (!install->isOk()) exit(1);
//
//  IsupCmrj* msgPtr = NEW IsupCmrj(msgSetId);
